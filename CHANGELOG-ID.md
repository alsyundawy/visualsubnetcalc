...
- Tombol Reset Perhitungan Subnet Instan (`#btn_reset`): Menambahkan tombol reset khusus berwarna merah pastel (`.btn-pastel-red`) di samping tombol Tools pada kontrol `#calc`. Mengembalikan seluruh kalkulasi secara instan ke kondisi awal (`10.0.0.0/16` untuk IPv4, `2001:db8::/32` untuk IPv6, mode operasi Standard), mereset status validasi form jQuery, memperbarui status tombol preset aktif, mereset indikator RFC 1918, merender ulang tabel subnet, serta menyinkronkan URL kueri.
- Standar Palet Warna Pastel 2026: Memperbarui skema warna tombol kontrol dengan standar palet pastel modern 2026. Tombol Tools menggunakan Hijau Pastel (`.btn-pastel-green`) dan Tombol Reset menggunakan Merah Pastel (`.btn-pastel-red`), direkayasa dengan rasio kontras tinggi standar WCAG AAA / AA pada mode terang dan gelap disertai transisi hover yang halus.
...
- Perbaikan Total Penyeragaman Gaya Tombol Footer (Memperbaiki Ikon Hitam WhatsApp, QRIS, dan Pengunjung): Memperbaiki anomali di mana tombol WhatsApp (`wa.me`), QRIS (`<button>`), dan Pengunjung (`<button>`) sebelumnya berikon hitam sementara tombol lainnya biru. Ditemukan akar masalah berupa selector CSS lawas `#app_footer a[href*="alsyundawy"]` yang hanya mengenai tautan dengan kata "alsyundawy". Telah digantikan dengan selektor universal `.footer-social-btn i` bernilai `#0284c7` (mode terang) dan `#38bdf8` (mode gelap) sehingga seluruh 9 tombol tampil seragam dan harmonis.
...
- Tombol Penghitung Pengunjung Dinamis (`#visitor_counter_btn`) & Integrasi `counter.txt`: Menambahkan tombol counter pengunjung interaktif yang terletak tepat di bawah bagian donasi PayPal / QRIS pada footer di `dist/index.html` dan `dist/404.html`. Menggunakan berkas `dist/counter.txt` sebagai baseline persisten yang selalu bertambah dinamis di setiap kunjungan baru via `localStorage`/`sessionStorage` serta sinkronisasi server otomatis. Dilengkapi pemformatan angka berstandar lokal (`id-ID`), fitur klik-untuk-memperbarui dengan animasi putar (`fa-spin`), dan styling pill yang selaras sempurna dengan tombol `.footer-social-btn`.
...
- Preset Blok IP Privat RFC 1918 & Indikator Real-Time: Menambahkan chip pemilih cepat untuk tiga alokasi blok IP privat IETF RFC 1918 (`10.0.0.0/8` blok 24-bit, `172.16.0.0/12` blok 20-bit, dan `192.168.0.0/16` blok 16-bit) dengan tetap mempertahankan preset default pada `/16` (`10.0.0.0/16`). Mengintegrasikan lencana status dinamis (`#rfc1918_indicator`) yang memverifikasi alamat jaringan aktif secara real-time terhadap batas alokasi RFC 1918.
- Optimalisasi Responsif Khusus Xiaomi, Redmi & POCO: Riset mendalam terhadap karakteristik render browser MIUI/HyperOS (inflasi ukuran font sistem, rasio layar panjang 20:9, notch punch-hole DotDisplay). Mengintegrasikan `viewport-fit=cover`, penanganan safe area `env(safe-area-inset-*)`, stabilisasi font `-webkit-text-size-adjust: 100%`, menghapus batasan kaku `min-width: 576px;`, dan scrolling sentuh halus (`-webkit-overflow-scrolling: touch`) pada kontainer tabel tanpa menyembunyikan kolom rentang atau usable IP.
...
- Tombol Media Sosial & Kontak Modern: merombak saluran kontak dan media sosial di footer serta navigasi menjadi kartu pill glassmorphism elegan (`.footer-social-btn`) dengan efek cahaya hover khusus tiap brand (GitHub, alsyundawy.com, X, Telegram, WhatsApp, Email, PayPal, QRIS) dan tombol aksi sirkular yang terpadu (`.nav-action-btn`).
...
- Suite Verifikasi Lintas Perangkat & Peramban: Pengujian otomatis headless Chrome dan Playwright memvalidasi 8 profil resolusi perangkat (VGA 640x480, Xiaomi Redmi A2 360x800, Redmi Note 13 392x872, POCO X6 Pro 412x915, iPhone 15 Pro, iPad Air, MacBook, Desktop FHD, dan Desktop 2K QHD), spasi vertikal otomatis, dan auto-deteksi URL path tanpa pemotongan dokumen horizontal.
