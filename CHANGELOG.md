...
- Instant Reset Button (`#btn_reset`): Added a dedicated, pastel red (`.btn-pastel-red`) reset button beside the Tools button in `#calc` controls. Instantly resets calculations back to standard default values (`10.0.0.0/16` for IPv4, `2001:db8::/32` for IPv6, Standard operating mode), clears jQuery validation errors, updates active presets, resets RFC 1918 indicator, re-renders table, and synchronizes URL query state.
- 2026 Pastel Palette Color Standards: Upgraded control button color scheme to modern 2026 pastel palette specifications. Tools button is styled in Pastel Green (`.btn-pastel-green`), and Reset button is styled in Pastel Red (`.btn-pastel-red`). Engineered with strict WCAG AAA / AA contrast ratios in both light and dark modes with subtle hover transitions.
