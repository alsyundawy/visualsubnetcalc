...
- **Dark Mode Dynamic Perceived Brightness & Contrast Engine (`isColorLight`)**: Perceived luminance calculation in `dist/js/main.js` using the ITU-R BT.601 formula ($0.299R + 0.587G + 0.114B > 140$) dynamically assigning `.has-light-bg` or `.has-dark-bg` to table rows. For custom colored rows in Dark Mode, forces high-contrast dark `#0f172a` text on light backgrounds and `#f8fafc` text on dark backgrounds, completely eliminating invisible text.
- **Bootstrap 5 Modal Dismiss Transition Race Condition Mitigation**: Element-scoped `_pendingDismiss` flag pattern on modal DOM nodes. Dismissal is safely queued until `shown.bs.modal` fires and cleared on `hide.bs.modal` or `hidden.bs.modal`, guaranteeing 100% reliable dismissal without event leakage during rapid automated test clicks or quick user interactions.
- **Back to Top Floating Action Button (`#btn_scroll_top`)**: Responsive floating circular action button positioned at the bottom-right corner with scroll monitoring (> 120px), smooth CSS opacity/transform transitions, hover micro-elevation, and hardware-accelerated smooth scrolling to top. Hardened DOM lifecycle timing guarantees reliable binding across IPv4 and deep IPv6 divisions.
- **Default Dark Mode on Initial Load & Anti-FOUC**: First-time visitors are automatically served the sleek, high-contrast Dark Mode theme inspired by `ns1.orion.net.id`, applied via inline `<script>` in `<head>` before CSS render tree construction.
- **100% English Code & UI Localization Policy**: Audited and confirmed 100% of production code, HTML, CSS, JavaScript, attributes, ARIA tags, labels, tooltips, modals, badges, and tests are strictly in English, with Indonesian documentation isolated into `*-ID.md`.
- **Interactive IPv6 Subnet Splitting from `/64` to `/127`**: Unlocked interactive splitting for `/64` through `/127` prefixes into subsequent tiers (e.g. `/68`, `/72`, `/80`, `/84`, `/88`, `/92`, `/96`, `/112`, and `/127`), enabling granular micro-segmentation down to point-to-point links (RFC 6164) with `/128` loopback boundary protection.
...
- **Preserved Dynamic Visitor Counter Pill (`#visitor_counter_btn`) & `counter.txt` Integration**: Preserved the Visitor Counter button as an interactive pill with text label in the footer column, fetching baseline count metrics from `dist/counter.txt` with cache-busting, dynamically tracking and incrementing visit counts across sessions, attempting server-side updates when supported, and providing interactive refresh animations (`fa-spin`) on click. Formatted with Indonesian / international numeral grouping.
- **WhatsApp & QRIS Geometry Consistency**: Standardized height, font family, font size, and border radius consistency between WhatsApp and QRIS circular buttons to pass all strict cross-browser geometry tests without rasterization blur.
...
- **CodeQL Alert #6 [High] DOM XSS Remediation**: Safe DOM element construction in `updateRfc1918Indicator`, sanitized `error.text()` extraction in form error placement, context-aware `escapeHtml()` in parent header rows, and eliminated dynamic `aria-label` attribute pollution.
...
- **Browser Compatibility & DX Hardening**: Resolved browser compatibility warnings for `text-size-adjust` via standard and `-webkit-` declarations alongside `.hintrc` configuration, resolved TypeScript definitions in `src/playwright.config.ts`, and updated the project demonstration recording (`src/demo.gif`).
- **Comprehensive Quality Automation & MegaLinter Integration**: Enabled full MegaLinter suite (`.mega-linter.yml`), root `eslint.config.js`, `.stylelintrc.json`, achieving 100% clean passes (0 errors, 0 warnings) across HTML, CSS, JavaScript, and Markdown.
