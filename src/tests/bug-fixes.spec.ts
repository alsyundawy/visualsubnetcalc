...
test("#22 - Broken Order/Render From URL Import", async ({ page }) => {
  await page.goto(
    "/index.html?c=",
  );
  await expect(
    page
      .getByLabel("10.0.0.0/15", { exact: true })
      .getByLabel("Subnet Address"),
  ).toContainText("10.0.0.0/15");
  await expect(
    page
      .getByLabel("10.2.0.0/15", { exact: true })
      .getByLabel("Subnet Address"),
  ).toContainText("10.2.0.0/15");
  await expect(
    page
      .getByLabel("10.4.0.0/15", { exact: true })
      .getByLabel("Subnet Address"),
  ).toContainText("10.4.0.0/15");
  await expect(
    page
      .getByLabel("10.6.0.0/16", { exact: true })
      .getByLabel("Subnet Address"),
  ).toContainText("10.6.0.0/16");
  await expect(
    page
      .getByLabel("10.7.0.0/16", { exact: true })
      .getByLabel("Subnet Address"),
  ).toContainText("10.7.0.0/16");
  await expect(
    page
      .getByLabel("10.8.0.0/13", { exact: true })
      .getByLabel("Subnet Address"),
  ).toContainText("10.8.0.0/13");
  await expect(
    page
      .getByLabel("10.16.0.0/13", { exact: true })
      .getByLabel("Subnet Address"),
  ).toContainText("10.16.0.0/13");
  await expect(
    page
      .getByLabel("10.6.0.0/16", { exact: true })
