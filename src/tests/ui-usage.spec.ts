...
test("CIDR Input Typing", async ({ page }) => {
  ...
  await expect(
    page.getByRole("cell", { name: "192.168.0.0/24 Subnet Address" }),
...
test("CIDR Input Paste", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Go" }).click();
  await expect(
    page.getByRole("cell", { name: "172.16.0.0/12 Subnet Address" }),
  ).toContainText("172.16.0.0/12");
});

test("About Dialog", async ({ page }) => {
  await page.goto("/");
  await page.locator("#info_icon").click();
  await expect(page.locator("#aboutModalLabel")).toContainText(
    "About Visual Subnet Calculator",
  );
  await expect(page.getByLabel("About Visual Subnet Calculator")).toContainText(
    "Design Tenets",
  );
  await expect(page.getByLabel("About Visual Subnet Calculator")).toContainText(
    "Credits",
  ...
  await expect(
    page.getByLabel("About Visual Subnet Calculator").getByText("Close"),
  ).toBeVisible();
});
...
test("Color Palette", async ({ page }) => {
  ...
  await expect(
    page.getByLabel("Stop Changing Colors").locator("span"),
  ).toContainText("« Stop Changing Colors");
...
test("Test Default Colors", async ({ page }) => {
  await page.goto(
    "/index.html?c=1N4IgbiBcIEwgNCARlEBGADAOm7g9GgGwIgDOUoGA5hSBgBa0YCWTAVkwNYUC+ia3SMB590HIbEHDEAZikjRaVhJjjQAFnmIArPNEy1IQlpAB2PSP6MVyjYYAcJgJx6dhzCbQDelkDNtG7jCecj6Ipu6avPy6PgoiQA",
  );
  await page.getByText("Change Colors »").click();
  ...
  await page.getByLabel("Color 1", { exact: true }).click();
  await page
    .getByRole("cell", { name: "10.0.160.0/20 Subnet Address" })
    .click();
  await expect(page.getByRole("row", { name: "10.0.160.0/20" })).toHaveCSS(
    "background-color",
    "rgb(255, 173, 173)",
  );
  await page.getByLabel("Color 2", { exact: true }).click();
  await page
    .getByRole("cell", { name: "10.0.176.0/20 Subnet Address" })
    .click();
  await expect(page.getByRole("row", { name: "10.0.176.0/20" })).toHaveCSS(
    "background-color",
    "rgb(255, 214, 165)",
  );
